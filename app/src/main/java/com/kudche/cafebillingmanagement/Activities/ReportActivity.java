...
public class ReportActivity extends AppCompatActivity {
    ...
    private void checkPermissionAndExport(String type) {
        // Android 10+ (Q) uses MediaStore/Scoped Storage, no WRITE permission needed for Downloads
    ...
    private void exportPDF() {
        ...
        canvas.drawText("Sales Report", 50, y, paint);
        
        y += 30;
        paint.setTextSize(12f);
        paint.setFakeBoldText(false);
        canvas.drawText("Date Range: " + displayFormat.format(new Date(startDate)) + " - " + displayFormat.format(new Date(endDate)), 50, y, paint);
        ...
        y += 20;
        paint.setFakeBoldText(false);
        canvas.drawText("Total Orders: " + currentDetailedOrders.size(), 50, y, paint);
        canvas.drawText("Total Items Sold: " + totalItemsGlobal, 250, y, paint);
        y += 20;
        canvas.drawText("Total Sales: Rs." + (int)totalSalesValGlobal, 50, y, paint);

        y += 40;
        paint.setFakeBoldText(true);
        canvas.drawText("Product Wise Sales", 50, y, paint);
        y += 20;
        paint.setFakeBoldText(false);
        canvas.drawText("Product", 50, y, paint);
        canvas.drawText("Qty", 250, y, paint);
        canvas.drawText("Total Sales", 450, y, paint);
        y += 10;
        canvas.drawLine(50, y, 550, y, paint);
        y += 20;
        for (SaleDao.ProductSalesReport p : currentProductReports) {
            canvas.drawText(p.productName, 50, y, paint);
            canvas.drawText(String.valueOf(p.quantitySold), 250, y, paint);
            canvas.drawText("Rs." + (int)p.totalSales, 450, y, paint);
            y += 20;
            if (y > 800) break; 
        }
        ...
        try {
            ...
            saveFileToDownloads("Sales_Report_" + System.currentTimeMillis() + ".pdf", "application/pdf", baos.toByteArray());
        } catch (IOException e) {
